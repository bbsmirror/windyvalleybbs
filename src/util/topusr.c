...
#define OUTFILE_TOPEVALUATE BBSHOME"/gem/@/@-topevaluate" //優質文章榮譽榜 
...
static DATA toplogins[TOPNUM], topposts[TOPNUM], topmoney[TOPNUM], topgold[TOPNUM], topevaluate[TOPNUM];
  ...
  ACCT *acct;
{
  ...
  if ((p = findmin(&topevaluate))->num < acct->goodart) //優質文章榮譽榜 
  {
    str_ncpy(p->userid, acct->userid, sizeof(p->userid));
    str_ncpy(p->username, acct->username, sizeof(p->username));
    p->num = acct->goodart;
...
main()
{
  ...
  qsort(topevaluate, TOPNUM, sizeof(DATA), sort_compare);
  write_data(OUTFILE_TOPEVALUATE, "優質文章榮譽榜", &topevaluate);
