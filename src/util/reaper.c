...
/* cache.080715: 目前設定只砍 登入不超過三次的使用者 及 被設定清除帳號 */ 

/* 保留帳號期限 -- 學期中 */
#define DAY_NEWUSR	365	    /* 登入不超過10次的使用者保留 365 天 */
#define DAY_FORFUN	1825	/* 未完成身分認證的使用者保留 365*5 天 */
#define DAY_OCCUPY	9999	/* 已完成身分認證的使用者保留 9999 天 */

/* 保留帳號期限 -- 暑假 */
#define VAC_NEWUSR	365	    /* 登入不超過10次的使用者保留 365 天 */
#define VAC_FORFUN	1825    /* 未完成身分認證的使用者保留 365*5 天 */
                            /* 注意會有使用者在暑假因認證過期而因此被砍帳號 */
#define VAC_OCCUPY	9999	/* 已完成身分認證的使用者保留 9999 天 */
...
#ifdef EADDR_GROUPING
/*
 * Thor.980930: 將同一email addr的account, 收集起來並列表 工作原理: 1.
 * _hash() 將 email addr 數值化 2.用binary search, 找到則append userno,
 * 找不到則 insert 新entry 3.將 userno list 整理列出
 * 
 * 資料結構: chain: int hash, int link_userno plist: int next_userno
 * 
 * 暫時預估email addr總數不超過100000, 暫時預估user總數不超過 100000
 * 
 */
  ...
  char *lowid;
{
  ...
  {
    if (login <= 10 && life < due_newusr) //登入10次以內又是newuser會砍 
      life = 0;
    ...
    /* itoc.010927: 有「禁止上站」的不砍，但若有「清除帳號」權限照砍 */
    {
      /* life = 1; */	/* 不需要，前面有了 */
    }
...
//    }
    ...
    {
      if (login <= 50 && life < due_forfun) //登入50次以內又是forfunuser會砍
	life = 0;
      else
	invalid++;
    }
...
/* itoc.011124: 不砍帳號: 已認證人的 acct.lastlogin 都 >= 0 */
  due_occupy = 0;
