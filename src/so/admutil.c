...
/*080907.cache: 反正每日小秘訣很少人在看就用預設值吧 ^.< */
...
is_school_email()
{
  ...
  if (HAS_PERM(PERM_VALID))  /* 避免用別的信箱認證再改成學校信箱但沒認證 */
  {
    ...
    *host++ = '\0';
    ...
    //mail 自訂                                                                                
    if (!strcmp(host, "mail.ncku.edu.tw") || !strcmp(host, "ccmail.ncku.edu.tw"))
      return 1;
  }
...
a_personal_board()
{
  ...
  ACCT acct;
  FILE *fp;
  char *fname = "gem/@/@Personal";      // 分類自定
                                                                                
#if 1   // 這一段是條件的檢查，可以隨便改或是拿掉
  if (!is_school_email())
  {
    vmsg("註冊之E-mail不合格或已申請過");
    return XEASY;
  }
                                                                                
  if (cuser.numlogins < 300)//設高一點避免被洗 
  {
    vmsg("上站次數不夠無法申請");
    return XEASY;
  }
  ...
  // 這裡是想讓個人板有什麼板名，下面二行只能選一行
  sprintf(brdname, "P_%.10s", cuser.userid);  /* 板名為 P_userid */
  
  /*cache.080805: 長ID (超過十個字元) 會被強制去掉 這是M3版面問題*/
  /*因為ID長度限制更動要改的太多了,可以讓此類使用者申請站務手動改板名*/
