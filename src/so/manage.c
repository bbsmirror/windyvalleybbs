...
/* cache.090526: 可設定是否能開啟優劣文 */
static int
post_battr_evaluate(xo)
  XO *xo;
{
  BRD *oldbrd, newbrd;
  ...
  switch (vans("開放優劣文文 (1)不許\ (2)允許\ (Q)取消？[Q] "))
  {
  case '1':
    newbrd.battr &= ~BRD_EVALUATE;
    break;
  case '2':
    newbrd.battr |= BRD_EVALUATE;
    break;
  ...
  XO *xo;
{
  ...
  switch (vans("開放推文 (1)不許\ (2)允許\ (Q)取消？[Q] "))
  {
  case '1':
    newbrd.battr &= ~BRD_SCORE;
    break;
  case '2':
    newbrd.battr |= BRD_SCORE;
    break;
...
return XO_NONE;
    ...
    pmsg("RSS訂閱\器測試中, 有任何問題請回報 SYSOP 板");
...
#else
  char *menu = "◎ 板主選單 (B)主題 (W)進板 (P)類別 (Z)RSS (U)記錄 "
#  ifdef HAVE_SCORE
    " (S)推文"
#  endif
#  ifdef HAVE_MODERATED_BOARD
