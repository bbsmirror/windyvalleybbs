...
#define GOLD2MONEY	1000000	/* 金幣→銀幣 匯率 */
#define MONEY2GOLD	1000000	/* 銀幣→金幣 匯率 */
...
b_cloak()
{
  ...
  else
  {
    if (cuser.gold < 9999)
    {
      vmsg("要 9999 金幣才能購買無限隱形權限喔");
    }
    else if (vans("是否花 9999 金幣購買無限隱形權限(Y/N)？[N] ") == 'y')
    {
      cuser.gold -= 9999;
...
b_mbox()
{
  ...
  else
  {
    if (cuser.gold < 9999)
    {
      vmsg("要 9999 金幣才能購買信箱無限權限喔");
    }
    else if (vans("是否花 9999 金幣購買信箱無限權限(Y/N)？[N] ") == 'y')
    {
      cuser.gold -= 9999;
      buy_level(PERM_MBOX);
    }
...
b_xempt()
{
  ...
  else
  {
    if (cuser.gold < 9999)
    {
      vmsg("要 9999 金幣才能購買帳號永久保留權限喔");
    }
    else if (vans("是否花 9999 金幣購買帳號永久保留權限(Y/N)？[N] ") == 'y')
    {
      cuser.gold -= 9999;
