...
// author : Zero <dinocho at gmail.com> 
// Version: 0.3
// create : 2007/01/08					 
// update : 2008/01/18	
// This program is licensed under new BSD License.
//-------------------------------------------------------
...
double BBSRUBY_INTERFACE_VER = 0.112;
int ABORT_BBSRUBY = 0;
...
VALUE bbs_getkey(VALUE self, VALUE wait)
{
    double data = NUM2DBL(wait);
...
VALUE bbs_printnum(VALUE self, VALUE var)
{
    return NUM2INT(STR2CSTR(var));
}
...
/*
                ...
                VALUE exception = rb_gv_get(" ");
               	char *ick;
                err_info_str = rb_obj_as_string(exception);
	...
	char* fpath;
{
	...
	if (!ruby_inited)
	{
        ...
        rb_define_singleton_method(rb_cBBS, "getkey", bbs_getkey, 0);
        rb_define_singleton_method(rb_cBBS, "printnum", bbs_printnum, 0);
		// Set safe level to 2
