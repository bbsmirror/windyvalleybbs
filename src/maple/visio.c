...
/* 090202.cache: 改吃 xdbbs 的 outx */

#ifdef SHOW_USER_IN_TEXT//新的控制碼 

static char deltatimebuf[50];

char *
delta_time_visio(Y,y,M,m,D,d)
char Y,y,M,m,D,d;
{
    struct tm then;
    time_t interval;
...
expand_esc_star_visio(char *buf, const char *src, int szbuf)
{
    ...
    switch(*++src)
    {
        ...
        case 'p':   // current user posts
            ...
            return 2;
        case 'g':   // current user gold
            snprintf(buf, szbuf, "%d", cuser.gold);
            return 2;
	    ...
	    case 'T':
	        ...
	        strlcpy(buf,delta_time_visio(src[1],src[2],src[3],src[4],src[5],src[6]),szbuf);
	        return 1;    
    }
  ...
  uschar *str;
{
  ...
  while (ch = *str)
  {
    ...
    if (ch == KEY_ESC && str[1] == '*')
    {
      ch = expand_esc_star_visio(buf, str, sizeof(buf));
      if (ch > 1)
         control = 1;
      outs(buf);
      str += (str[2] == 'T' && ch ? 8 : 2);
  ...
  uschar *str;
{
  ...
  while (ch = *str)
  {
#if 1   /* itoc.020301: 讓 ESC + * + s 等控制碼在文章編輯/閱讀也適用 */
    if (ch == KEY_ESC && str[1] == '*')
    {
      switch (str[2])
      {
