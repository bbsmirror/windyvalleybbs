  ...
  char *title, *mid;
{
  ...
  if ((rec_num(FN_RUN_RFORM, sizeof(RFORM)) > 0) && (HAS_PERM(PERM_ALLREG)))
  {
    mid = "\033[5;41m 註冊單待審核 \033[m";
    spc = 14;
  }
...
/* cache.080307: 暫時不予開放 */ 
  ...
  "bin/bank.so:b_cloak", PERM_SYSOP, - M_GAME,
  "Cloak      無限隱形",

  "bin/bank.so:b_mbox", PERM_SYSOP, - M_GAME,
  "Mbox       信箱無限",

  "bin/bank.so:b_xempt", PERM_SYSOP, - M_GAME,
  "Xempt      永久保留",
