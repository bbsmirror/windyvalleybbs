  ...
  BRD *brd;
{
  ...
  while (1)
  {
    ...
    if (read(fd, &chrono, sizeof(HDR)) == sizeof(HDR))
    {
...
XZ xz[] =
{
  ...
  {NULL, rss_cb, M_XMODE, FEETER_RSS}      /* XZ_RSS */
...
//todo: to be modified
static void rss_preview(XO *xo, int zone)
{
    rssfeed_t *feed;
...
return;
    ...
    feed = (rssfeed_t *) xo_pool + (xo->pos - xo->top);

    sprintf (fpath, FN_RSS_FOLDER "%s", escape_slash(feed->feedurl));
    if (rec_get(fpath, &brdlist, sizeof(rssbrdlist_t), 0) < 0)
brdlist.update = 0;
    ...
    move(b_lines - 3, 0);
    clrtoeol();
    prints("\033[1;33m建立日期:\033[m %s ", Btime(&(feed->chrono)));
    prints("(最近抓取 -> %s)"
    , (brdlist.update <= 0 ? "尚未更新" : Btime(&(brdlist.update))));

    move(b_lines - 2, 0);
    clrtoeol();
    prints("\033[1;33m屬性設定:\033[m [%s]站際存檔 [%s]發文類別 [%s]下載更新 [%s]停止抓取"
    , feed->attr & RSS_OUTGO ? "○" : "╳"
    ...
    move(b_lines - 1, 0);
    clrtoeol();
    prints("%-*.*s", b_cols - 1, b_cols - 1, feed->feedurl);
    outf(xz[zone - XO_ZONE].feeter);
    move(b_lines, b_cols);
}
...
//     if (zone==XZ_CLASS || zone==XZ_MF || zone==XZ_RSS)
//       XO_TALL = IN_CLASS_ROOT(xo) ?  b_lines - 10 : b_lines - 3;
//     else
//	   XO_TALL = (b_lines - 3);
