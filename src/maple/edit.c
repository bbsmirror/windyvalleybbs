 ...
 while(vln)
 {
   ...
   if(!strcmp(head,"\033[1;36m"))
   {
	  ...
	  strcpy(cmp,reason);
          ...
          if (!vget(b_lines, 0, "修  改  推  文  ：", reason, len, GCARRY))
            cc=1;
      ...
      case Ctrl('Q'):
        {
          ...
          if (cc > '0')
          {
            if (cc != '9')
              ...
              {
                pmsg("請依序輸入欲倒數計時的日期之 年/月/日");
                if (!vget(b_lines, 0, "倒數計時 - 年(2000 ~ 2099): 20", datebuf[0], 3, DOECHO))
                  break;
                if (atoi(datebuf[0]) < 0)
                  break;
                if (!vget(b_lines, 0, "倒數計時 - 月(1 ~ 12): ", datebuf[1], 3, DOECHO))
                  break;
                if (atoi(datebuf[1]) > 12 || atoi(datebuf[1]) < 1)
                  break;
                if (!vget(b_lines, 0, "倒數計時 - 日(1 ~ 31): ", datebuf[2], 3, DOECHO))
                  break;
                if (atoi(datebuf[2]) > 31 || atoi(datebuf[2]) < 1)
                  break;
