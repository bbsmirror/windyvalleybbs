...
/* this version of bhttpd.c is modified by cache
 * cache.bbs@bbs.ee.ncku.edu.tw
 *
 * It sohuld be OK to compile by gcc4.3 for M3-itoc BBS
 * 
 * Rev log
 *
 * 20090405 - guest can't get readlevel
 *          - POST_RESTRICT / POST_LOCKED won't show on the list
 *          - mainpage_neck modified, disallow robot search
 *
 * 20090415 - more_item xmode fix
 *
 */
  ...
  int start, total;
{
  ...
  if ((fd = open(folder, O_RDONLY)) >= 0)
  {
	...
	*ptr2 = '\0';
    ...
    fprintf(fpw, "%s</td>\n</tr>\n",
    ((hdr.xmode & POST_RESTRICT) || (hdr.xmode & POST_LOCKED)) ? "<此篇文章已加密/鎖定>" : str_html(hdr.title, 50));
#else
...
#ifdef HAVE_REFUSEMARK
      if ((hdr.xmode & POST_RESTRICT) || (hdr.xmode & POST_LOCKED))
	    out_mesg(fpw, "這是加密/鎖定的文章，您無法閱\讀");
      else
      {
#endif          
