...
//for M3_USE_PFTERM
#define FILEDFG            (0)         /* 輸入框的前景顏色 (以 0 - 7 表示) */
...
#define COLOR_OGOOD	"\033[1;35m"	/* 與我為友 */
#define COLOR_CLOAK	"\033[1;34m"	/* 隱形 */	/* itoc.註解: 沒用到，要的人請自行加入 ulist_body() */
#define COLOR_SELF	"\033[1;33m"	/* 自己 */
#define COLOR_BOTHGOOD	"\033[1;37m"	/* 互設好友 */
#define COLOR_BRDMATE	"\033[1;36m"	/* 板伴 */
...
#define NECKER_CLASS	"[←]主選單 [→]閱\讀 [↑↓]選擇 [c]篇數 [y]載入 [/?]搜尋 [s]看板 [h]說明\n" \
			COLOR3 "  %s   看  板       類別 ◎ 中   文   敘   述%*s              人氣 板    主%*s    \033[m"
...
#define NECKER_XPOST	"\n" \
			COLOR3 "  編號    日 期 作  者       文  章  標  題%*s                            推:%s  \033[m"

#define NECKER_MBOX	"[←]離開 [→,r]讀信 [d]刪除 [R,y](群組)回信 [s]寄信 [x]轉錄 [F]轉達 [h]說明\n" \
			COLOR3 "  編號   日 期 作  者       信  件  標  題%*s                                    \033[m"

#define NECKER_POST	"[←]離開 [→]閱\讀 [^P]發表 [b]進板畫面 [d]刪除 [V]投票 [TAB]精華區 [h]說明\n" \
			COLOR3 "  編號      日 期 作  者       文  章  標  題%*s                推:%s  人氣:%-4d  \033[m"
...
#define NECKER_RSSADMIN	"[←]離開 [U]更新 [/]搜尋 [Enter]詳細\n" \
			COLOR3 "  編號            看         板%*s                                               \033[m"
...
COLOR1 " 魚雁往返 " COLOR2 " (ry)回信/群組 (M)轉達 (d)刪除 (m)標記 (C)暫存 (=\\[]<>-+;'`|?QAkj)  "
...
#define FEETER_MBOX	\
COLOR1 " 信信相惜 " COLOR2 " (y)回信 (F/M/x)轉寄/轉達/轉錄 (d)刪除 (D)區段刪除 (m)標記 (E)編輯  "

#define FEETER_POST	\
COLOR1 " 文章列表 " COLOR2 " (r/y)回信 (S/a)搜尋/標題/作者 (@)自動播放 (x)轉錄 (V)投票 (u)新聞  "
...
COLOR1 " 全站訂閱\ " COLOR2 " (U)更新列表 (/)搜尋 (→)網址列表                                    "
...
#define EDIT_BANNER	"\n--\n" \
			"\033[1;32m□ Origin:\033[m \033[1;33m風之谷\033[m \033[1;37m"MYHOSTNAME"\033[m \033[1;31m□ From:\033[m \033[1;37m%.32s\033[m\n"

//□			" \033[1;43m◤\033[46m◥\033[m Or\033[1mig\033[30min\033[m: \033[1;44m "SCHOOLNAME"˙"BBSNAME" \033[42m "MYHOSTNAME" \033[m\n" \
//			" \033[1;44m◣\033[41m◢\033[m A\033[1mut\033[30mho\033[mr: \033[1;33m%s\033[m 從 \033[1;31m%s\033[m 發表\n"

#define MODIFY_BANNER	"\033[1;32m◆ Modify:\033[m \033[1;37m%s\033[m \n"
...
#define ICON_UNREAD_BRD		"\033[33mˊ\033[m"	/* 未讀看板 */
